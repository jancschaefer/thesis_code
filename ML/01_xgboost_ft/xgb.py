...
data = pq.read_table(dataPath + '/converted.ft.dropped.parquet').to_pandas()
...
# create dummies
XD = pd.get_dummies(X,prefix=["CI","BVD"],prefix_sep="_",columns=['Country_Iso','Bvd_Independence'],dummy_na=True)


# %% train/test split
seed = 7
test_size = 0.33
X_train, X_test, y_train, y_test = train_test_split(XD, Y, test_size=test_size, random_state=seed)
