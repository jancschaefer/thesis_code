	...
	for index, firm in currentData.iterrows(): # for every single company
		...
		for word in NaceDescription: # for every word => build vector
			try:
				...
				continue
		logger.info('(%s) Working on: %s => %s entries', index, firm['NACE_Code'], len(df) )
...
nace_codes.loc[:,'Description'] = nace_codes.loc[:,'Description'].str.lower().str.split()  # split every trade description into single words
nace_codes.loc[:,'Description'] = nace_codes.loc[:,'Description'].apply(lambda x: [item for item in x if item not in stop])  # for every cell remove stop words
...
nace_codes.loc[:,'Description'] = nace_codes.loc[:,'Description'].apply(lambda x: [lem.lemmatize(item) for item in x])  # reduce words to stems; e.g. products => product
...
# %% write xls
nace_worked.to_excel(dataPath + 'nacecodes.w2v.xlsx')
pq.write_table(pa.Table.from_pandas(nace_worked), dataPath + '/nacecodes.w2v.parquet')
...
item1 = nace_worked.loc[656]
item2 = nace_worked.loc[656]
print(vecDistance(item1,item2,prefix = 'DESC'))