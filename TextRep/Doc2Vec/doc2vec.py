...
sentence1 = TaggedDocument(
    words=set([u"raise", u"of", u"dairy", u"cattle"]), tags=set(["doc1"])
)
sentence2 = TaggedDocument(words=set([u"raise", u"of", u"pig"]), tags=set(["doc2"]))
sentence3 = TaggedDocument(
    words=set([u"repair", u"of", u"motorcycle"]), tags=set(["doc3"])
)
...
print(
    "Cattle & Cattle: \t",
    scipy.spatial.distance.euclidean(vec1, vec1_1),
    " \t| expected 0",
)
print(
    "Cattle & Pig: \t\t",
    scipy.spatial.distance.euclidean(vec1, vec2),
    " \t| expected low",
)
print(
    "Motorc & Trucks: \t",
    scipy.spatial.distance.euclidean(vec3, vec4),
    " \t| expected lowest",
)
print(
    "Cattle & Motorcycles: \t",
    scipy.spatial.distance.euclidean(vec1, vec3),
    " \t| expected high",
)
print(
    "Cattle & Trucks: \t",
    scipy.spatial.distance.euclidean(vec1, vec4),
    " \t| expected high",
)

# %% save
model.save(dataPath + "doc2vec_2000.model")
